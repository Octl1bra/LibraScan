...
代码以 [Apache License 2.0](LICENSE) 开源。你可以自由使用、修改、再分发，包括商用，只需保留版权与许可声明（[NOTICE](NOTICE)）。提交的贡献默认按同一许可授权（第 5 条），不需要单独签 CLA。

「LibraScan」这个名字和 app 图标不在许可范围内（第 6 条）：代码随便用，别顶着这个名字和图标上架。
